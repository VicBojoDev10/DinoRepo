...
public class PlayerManager : MonoBehaviour
{
    ...
    private bool isTakingDamage = false; // Bloquea acciones mientras está dañado
    ...
    private void Update()
    {
        ...
        // Comprobación simple de suelo (puedes ajustarlo con Raycast si lo prefieres)
        isGrounded = Physics2D.OverlapCircle(groundCheck.position, 0.2f, groundLayer);

        // Inputs de prueba (puedes llamarlos desde tus botones UI)
        if (Input.GetKeyDown(KeyCode.Space)) Jump();
        if (Input.GetKeyDown(KeyCode.X)) Attack();
    }

    // Método para ser llamado por tu UI (JumpButton)
    public void Jump()
    {
        if (isGrounded && !isTakingDamage)
        {
            ...
            // Llamar al manager de logros para sumar un salto
            AchievementManager.Instance?.AddJump();
        }
    }

    // Método para ser llamado por tu UI (SlashButton)
    public void Attack()
    {
        if (currentCooldown <= 0f && !isTakingDamage)
        {
            ...
            {
                hitSomething = true;
                // Aquí llamarías al script del enemigo para destruirlo
                // enemy.GetComponent<WormEnemy>().Die();
            }

            if (hitSomething)
            {
                currentCooldown = 0f; // Reinicio instantáneo si aciertas
            }
            else
            {
                currentCooldown = scratchCooldown; // Penalización si fallas
            }
        }
    }
